    ...
    <div class="form-container">
        ...
        <form action="{{ route('paralelos.update', $paralelo->id) }}" method="POST">
            ...
            <div class="form-group">
                ...
                <input type="text" id="nombre" name="nombre" value="{{ $paralelo->nombre }}" required>
            </div>
            <div class="form-group">
                <label for="cantidad_est">Cantidad de Estudiantes:</label>
                <input type="number" id="cantidad_est" name="cantidad_est" value="{{ $paralelo->cantidad_est }}" required>
            </div>
            <div class="form-group">
                <label for="curso_id">Curso:</label>
                <select id="curso_id" name="curso_id" required>
                    @foreach($cursos as $curso)
                        <option value="{{ $curso->id }}" @if($curso->id == $paralelo->curso_id) selected @endif>{{ $curso->nombre }}</option>
                    @endforeach
                </select>
            </div>
